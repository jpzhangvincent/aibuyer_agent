...
from langchain_upstage import ChatUpstage, GroundednessCheck
...
def get_retriever(index_path, model_name = "Alibaba-NLP/gte-base-en-v1.5"):
    ...
    return retriever
retriever = get_retriever(MONGODB_URI) #"../data/chroma_db"
...
def get_question_router():
    ...
    # Prompt 
    system = """You are an AI assisted buyer agent expert at routing a user question to a vectorstore for listing search or web search for general personalization advice.
    The vectorstore contains documents related to the property listing scraped from zillow inlcuding information like location, sqfeet, pricing, etc.
    Use the vectorstore for questions on these topics. For all else, for example, hoursing market analysis/advice, neighborhood/city information, use web-search."""
...
def grade_generation_v_documents_and_question(state):
    ...
    grade = hallucination_grader.invoke({"context": documents, "answer": generation})
    #grade = score.binary_score
